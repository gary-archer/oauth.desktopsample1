...
export class ErrorHandler {
     ...
     * Return an object for Ajax errors
     */
    public static getFromHttpError(exception: any, url: string): UIError {
        ...
        if (statusCode === 0) {
            ...
            error = new UIError(
                ...
                'A network problem occurred when the UI called the API',
                exception.stack);
            ...
            error = new UIError(
                'JSON',
                ErrorCodes.jsonDataError,
                'A technical problem occurred parsing data from the API',
                exception.stack);
            error.details = this._getExceptionMessage(exception);
            ...
            error = new UIError(
                ...
                'An error response was returned from the API',
                exception.stack);
            error.details = this._getExceptionMessage(exception);
